...
class SensorPageNew extends StatefulWidget {
  // final BluetoothConnection? connection;

  const SensorPageNew({Key? key}) : super(key: key);

  @override
  State<SensorPageNew> createState() => _SensorPageNewState();
}

class _SensorPageNewState extends State<SensorPageNew> {
  Timer? _timer;
  String _dist = "0.00";
  ...
  void _startListening() {
    final provider = Provider.of<BluetoothConnectionProvider>(context, listen: false);
    double prevDist = 0.00;
    provider.connection?.input!.listen((Uint8List data) {
  ...
  void _sendMessage(String message) async {
    final provider = Provider.of<BluetoothConnectionProvider>(context, listen: false);
    provider.connection?.output.add(Uint8List.fromList(utf8.encode(message)));
